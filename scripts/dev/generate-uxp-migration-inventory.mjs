...
function fallback(source, name) {
  if (name.startsWith('photoshop_recipe_')) return 'n/a — Guard denies recipe execution';
  const cls = migrationClass(source, name);
...
function parityStrategy(source, name) {
  if (name.startsWith('photoshop_recipe_')) {
    return 'Guard exclusion regression; recipe behavior is outside canonical painting acceptance';
  }
  const cls = migrationClass(source, name);

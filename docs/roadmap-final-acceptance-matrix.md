...
| Task 3 — CoS attribution/rebind survival | **live-pass** | Current slot-b `5d87e8d` live trace first observed real `unattributed` CoS execution without connector loss, then exact `request_id` recovery. After a built-in non-CoS web read, Core + `photoshop_ping` remained exactly attributed and `photoshop_guard_resume(3766)` recovered the pre-existing Task21a run/anchor state without replay. Hashed evidence: `processes/p0a-host-route-process/run-01/evidence/task3-rebind-live-acceptance.json`. |
...
| Task 4 — three-state host recovery model | **repo-pass + live-support** | CoS commit `b1ce9ed` explicitly distinguishes `Tool not selected` / `Caller unattributed` / `Connector genuinely unavailable` on Core and Plugins initialization surfaces and permits the last label only after a concrete discovery/readiness/tool-call failure. The Photoshop MCP instructions already require existing-surface → Guard status/resume → durable-state recovery. Task 3 live evidence proves resume preserved the current Task21a document/run/anchor state without replay. CoS focused validation: 190 passed / 6 skipped plus TypeScript. |
...
| Task 21a — one-action accepted-anchor restore | **live-pass** | Repository contract is covered by `accepted-anchor-restore.test.ts`. Current-build disposable run `processes/task21a-live-restore-process/run-02/` registered anchor `task21a-live2-anchor-01` at SHA `addeed28f…`, executed two later region mutations / two Photoshop history states, then issued one compact restore request containing only the anchor identity. Guard computed `undo(2)`, returned the exact anchor SHA, verified layer/active-layer/selection parity, recorded report/receipt/verdict internally, replayed no mutation, and left global Guard debt empty. Run-01 diagnostically exposed capture-profile-dependent JPEG identity; fix `7502987` made restore inherit the registered anchor capture spec before accepted run-02. Hashed summary: `processes/task21a-live-restore-process/run-02/evidence/task21a-live-acceptance.json`. |

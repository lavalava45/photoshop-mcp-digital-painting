...
| `problem_id` | compact pass schema; optional when request_key intentionally also names the problem | compiler preserves stable artistic problem identity independently of operation id | repeated-attempt/significance/strategy gates group by problem identity | operation journal / painting problem state | compact docs + painting skill | keep; distinct new attempts use new request_key with same problem_id |

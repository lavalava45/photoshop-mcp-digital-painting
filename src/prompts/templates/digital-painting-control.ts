...
export const digitalPaintingControlTemplate: PhotoshopPromptTemplate = {
  ...
  description:
    'Guide an agent through subject-agnostic digital painting using shape, value, form, edge, material and detail hierarchy, with visual checkpoints, cleanup and state-based completion criteria.',
  arguments: [
    {
      name: 'subject',
  ...
  handler: (args) => {
    ...
    const text = [
      ...
      ``,
      `Core painting discipline — subject agnostic:`,
      `- Treat painting as an iterative perceive → plan → paint → inspect → correct loop. Technical tool success is not visual success.`,
      `- Use the universal hierarchy SHAPE → VALUE → FORM → EDGE → MATERIAL → DETAIL. This applies to an apple, a house, a vehicle, a landscape, a figure, or an abstract painted object; do not substitute a subject-specific recipe unless the user asks for one.`,
      `- BLOCK-IN IS TEMPORARY. Large opaque brushes and simple geometric masses are appropriate for initial coverage, silhouette and composition, but do not keep describing the image with the same oversized primitive language after the block-in. Reduce brush scale as soon as the large relationships read.`,
      `- Choose brush size relative to the current form and canvas, not from a fixed pixel habit. Large brushes establish masses; medium brushes model planes/forms; small brushes resolve selected edges and details. Avoid using a brush whose diameter is comparable to a small feature unless deliberately filling that feature.`,
      `- Establish VALUE structure before chasing color nuance or detail. Separate the major light family, shadow family and important value groups first; color should support the value design rather than conceal a flat value structure.`,
      `- Model FORM by describing plane turns and gradual changes across the surface. Build volume with overlapping strokes, controlled opacity/flow and directional marks that follow the form; do not merely outline a flat shape and fill it.`,
      `- Control EDGES deliberately. Use a hierarchy of hard/firm/soft/lost edges according to focus, depth, lighting and material. Do not give every contour the same hardness, width or contrast. Lost and soft edges are often as important as drawn contours.`,
      `- Render MATERIAL from how light behaves on the surface: matte vs glossy, rough vs smooth, translucent vs opaque, metallic vs dielectric, textured vs uniform. Material is not just a different RGB fill; it changes highlight shape, edge behavior, contrast, reflection and texture.`,
      `- DETAIL comes last. Do not decorate unresolved flat masses. Add small accents only where they clarify focal structure, material or scale.`,
      `- Blending is not synonymous with blur. Prefer controlled overlap of lower-opacity/lower-flow strokes, intermediate values/colors, scumbling, hatching or textured strokes where appropriate. Use Smudge sparingly and intentionally; do not globally smear away useful edge structure.`,
      `- Avoid a vector/collage look unless the requested style calls for it: vary edge quality, stroke direction, opacity/flow, local value transitions and texture; avoid assembling the final image from uniformly hard opaque blobs, identical-width curves and perfect geometric fills.`,
      `- New strokes must account for existing forms, depth order and occlusion. Do not draw through foreground forms unless the overlap is intentional. Prefer short, intentional strokes when a path crosses complex geometry.`,
      `- Do not continue detailing on top of a visible structural, value, form or edge error. Correct the earlier stage first.`,
      `- For reference-driven color work, use \`photoshop_sample_color\` on the pinned reference document instead of guessing RGB values. Use radius=0 for a precise point and a small radius for a representative local average; treat samples as evidence, not as a substitute for value/form judgment.`,
      `- For proportion-sensitive work (architecture, perspective, repeated geometry, anatomy or any other measured subject), use a measurement checkpoint when useful: choose visible semantic landmarks, optionally add/list guides, call \`photoshop_measure_points\`, and correct structure before detailing.`,
      ...
      ``,
      `Workflow — advance only when the previous stage reads:`,
      ...
      `2. PLAN — identify composition, major shape masses, focal area, depth order, light direction/value grouping, likely materials and semantic passes. Do not hardcode subject-specific protected-region coordinates before seeing the actual image unless the user supplied explicit geometry/masks.`,
      `3. SHAPE / BLOCK-IN — establish silhouette, negative-space relationships and large masses with broad economical strokes. This is the only stage where very large opaque primitive-like marks should dominate unless the target style explicitly requires them.`,
      `4. CHECKPOINT — preview. Judge composition, silhouette, proportion and negative space. Fix must-fix shape problems now; do not hide them under rendering.`,
      `5. VALUE — establish the main light/shadow families and important value groups using broad-to-medium strokes. Preview and verify that the image reads in value before adding small detail.`,
      `6. FORM — turn the large masses into volumes with plane changes, halftones, core/cast shadows, reflected light or equivalent form cues appropriate to the lighting/style. Use medium and smaller brushes plus controlled opacity/flow rather than only opaque fills.`,
      `7. EDGE — inspect and intentionally vary hard/firm/soft/lost edges. Soften or lose edges where forms turn away, recede, merge or are out of focus; reserve crisp/high-contrast edges for structural or focal needs.`,
      `8. MATERIAL / COLOR — refine hue/temperature, reflections, highlight character, roughness/texture and other surface behavior without breaking the established value/form structure. For reference-driven work, sample a compact palette from the pinned reference with photoshop_sample_color rather than inventing colors blindly.`,
      `9. DETAIL / ACCENTS — reduce brush size and add only selected focal details, small texture, hatching, specular accents or line work that clarifies the image. Do not revert to outlining every form.`,
      `10. CLEANUP — repair accidental crossings, awkward tangencies, duplicated contours, floating marks, broken silhouettes, muddy blends, uniformly hard edges, inconsistent line weight and any remaining vector-like primitive artifacts not required by style. Use eraser/repainting when needed.`,
      `11. FINAL CHECK — inspect at overall, medium and detail scale. The image should work first as masses/values, then as forms/edges/materials, and only lastly as detail.`,
      ``,

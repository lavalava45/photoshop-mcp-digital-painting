...
export const digitalPaintingControlTemplate: PhotoshopPromptTemplate = {
  ...
  handler: (args) => {
    ...
    const text = [
      ...
      `- Prefer photoshop_execute_visual_microplan when setup/read/config steps plus ONE approved visual mutation can be collapsed safely: it must end immediately in photoshop_get_preview and may not contain a second visual mutation. Inspect the returned preview and pass its SHA + verdict/disposition as previous_preview before another VisualMicroPlan; the server barrier rejects an unclassified next bundle.`,
      `- A VisualMicroPlan mutation error is not permission to retry: the executor still captures the reconciliation preview because AUTO/chunked painting may have partially changed the canvas. Classify that frame first, then correct/rollback/replan.`,

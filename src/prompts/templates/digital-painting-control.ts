...
export const digitalPaintingControlTemplate: PhotoshopPromptTemplate = {
  ...
  handler: (args) => {
    ...
    const text = [
      ...
      ``,
      `- CYCLE: next_pass={request_key,problem_id?,document_id,goal,region/protection,action_class?,actions}. request_key=idempotent attempt; problem_id=stable problem. Continue with previous_operation_id + previous_observation + next_pass; previous_observation is model-owned. Omit next_pass to finalize. Guard acknowledges the exact stored receipt internally.`,
      `GUARD / CONTROLLER:`,
      `- OPEN BRIEF: when subject is delegated, compare 4–6 candidates from at least four families. Reject repeats and atmosphere-only premises; choose subject+setting+action+spatial problem. Ease never decides alone. Never browse prior runs for inspiration. No Guard gate or repeat inside the paint loop.`,
      ...
      `- LAYER SEPARATION CHECK: substantial independently-correctable object/material/light/plane uses create-new/temporary-hypothesis, never layer per stroke. Accepted ids use protected_layer_ids; protected_regions is descriptive only; replacement needs replace_protected_layer_ids.`,
      `- COMMENTARY: sticky "режим техника|художник|вместе" + detail. In artistic/mixed mode emit the root goal before each visual pass; compiler reuses it as artistic_commentary. Artist voice is natural Russian; no hidden chain-of-thought or infrastructure jargon.`,
      ...
      `- Structure before texture: fix thumbnail/blur structure first. paint_regions is broad scaffold in early stages; later it is allowed only for a bounded explicit REPLACE/ERASE correction with exact layer targeting and clip_bounds, never as continued flat ADD block-in.`,
